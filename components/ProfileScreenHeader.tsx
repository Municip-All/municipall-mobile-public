  ...
  return (
    <View
      ...
      <View className='flex-row items-center'>
        <TouchableOpacity onPress={() => router.back()} className='mr-3 p-2' accessibilityLabel='Retour'>
          <Ionicons name='chevron-back' size={24} color={dark ? '#FFF' : '#000'} />
        </TouchableOpacity>
        <Text className={`flex-1 text-lg font-bold ${dark ? 'text-white' : 'text-black'}`}>{title}</Text>
      </View>
      <Text className={`ml-11 mt-1 ${classes.meta}`}>Réglages du compte</Text>
    </View>
  );
}
