  ...
  return (
    <View className={`flex-1 ${dark ? 'bg-zinc-950' : 'bg-[#F8FAFC]'}`}>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }} bounces={false}>
        {/* Header Background */}
        <View 
          className="bg-[#244EE5] w-full px-6 pb-24"
          style={{ paddingTop: Math.max(insets.top, 40), borderBottomLeftRadius: 32, borderBottomRightRadius: 32 }}
        >
          <Text className="text-white text-3xl font-bold tracking-tight">Municip'All</Text>
        ...
        <View className="px-4 -mt-16">
          <View className={`rounded-[24px] overflow-hidden shadow-xl ${dark ? 'bg-zinc-900 border border-zinc-800' : 'bg-white border border-gray-100'}`}
            ...
            <View className="bg-[#1D4ED8] px-5 py-4 flex-row items-center justify-between">
            ...
            <View className="p-2">
              ...
              <TouchableOpacity className={`flex-row items-center p-4 rounded-2xl ${dark ? 'bg-zinc-800/50' : 'bg-transparent'}`}>
                <View className="w-12 h-12 rounded-full bg-blue-50 dark:bg-blue-900/30 items-center justify-center mr-4">
                  <Ionicons name="cloud-outline" size={24} color={dark ? '#60A5FA' : '#2563EB'} />
                </View>
              ...
              <TouchableOpacity className={`flex-row items-center p-4 rounded-2xl ${dark ? 'bg-zinc-800/50' : 'bg-transparent'}`}>
                <View className="w-12 h-12 rounded-full bg-blue-50 dark:bg-blue-900/30 items-center justify-center mr-4">
                  <Ionicons name="car-outline" size={24} color={dark ? '#60A5FA' : '#2563EB'} />
                </View>
                <View className="flex-1">
              ...
              <TouchableOpacity onPress={() => router.push('/events')} className={`flex-row items-center p-4 rounded-2xl ${dark ? 'bg-zinc-800/50' : 'bg-transparent'}`}>
                <View className="w-12 h-12 rounded-full bg-blue-50 dark:bg-blue-900/30 items-center justify-center mr-4">
                  <Ionicons name="people-outline" size={24} color={dark ? '#60A5FA' : '#2563EB'} />
                </View>
                <View className="flex-1">
        ...
        <View className="px-5 mt-8">
          ...
          <View className={`rounded-[20px] p-5 mb-4 shadow-sm border ${dark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-gray-100'}`}>
            <View className="flex-row">
              <Ionicons name="alert-circle-outline" size={22} color={dark ? '#60A5FA' : '#244EE5'} className="mr-3" />
      ...
      <View className="absolute bottom-24 right-5 items-end">
        <View className="flex-row items-center bg-white/80 dark:bg-black/50 backdrop-blur-xl rounded-full pr-1 pl-4 py-1 shadow-lg border border-white/20 dark:border-white/10">
          <Text className={`text-xs font-medium mr-3 ${dark ? 'text-white' : 'text-[#244EE5]'}`}>Vigilance Citoyenne</Text>
          <TouchableOpacity 
            onPress={() => router.push('/carte')}
            className="w-12 h-12 rounded-full bg-[#244EE5] items-center justify-center shadow-md">
