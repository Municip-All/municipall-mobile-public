...
import { View, Text, TouchableOpacity, TextInput, Image, KeyboardAvoidingView, Platform, ScrollView, ActivityIndicator, Alert } from 'react-native';
...
export default function Carte() {
  ...
  const pickImage = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Permission refusée', "Nous avons besoin de l'accès à vos photos pour joindre une preuve.");
      return;
    }
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <View className={`flex-1 ${dark ? 'bg-zinc-950' : 'bg-white'}`}>
        ...
        {/* Floating Header */}
        <View
          className="w-full shadow-lg z-10 p-5 rounded-b-[24px]"
        ...
        <View className="absolute bottom-[90px] w-full items-center z-10 px-6">
          <TouchableOpacity
            ...
            className="flex-row items-center justify-center rounded-full py-4 px-8 shadow-2xl w-full"
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row mb-4">
                {categories.map((cat) => (
                  <TouchableOpacity 
                    ...
                    className={`px-4 py-2 rounded-full border border-gray-200 mr-2 flex-row items-center ${category === cat ? 'bg-blue-500 border-blue-500' : (dark ? 'bg-zinc-800 border-zinc-700' : 'bg-white')}`}
                    style={category === cat ? { backgroundColor: primaryColor, borderColor: primaryColor } : {}}
                  >
                    <Text className={category === cat ? 'text-white font-medium' : (dark ? 'text-gray-300' : 'text-slate-800')}>{cat}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
